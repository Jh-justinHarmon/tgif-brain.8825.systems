...
# Dropbox API client (for Replit integration)
def get_dropbox_client():
    """Get Dropbox client if token available."""
    token = os.environ.get("DROPBOX_TOKEN") or os.environ.get("REPLIT_DB_DROPBOX_TOKEN")
    if token:
        try:
            import dropbox
...
def load_from_dropbox(dbx, dropbox_path: str) -> Optional[str]:
    """Load file content from Dropbox."""
    try:
        metadata, response = dbx.files_download(dropbox_path)
        return response.content.decode('utf-8')
    except Exception as e:
        print(f"Dropbox download failed for {dropbox_path}: {e}", file=sys.stderr)
        return None

def load_capability_map() -> dict:
    """Load capability map from Dropbox or local filesystem."""
    # Try Dropbox API first (Replit)
    dbx = get_dropbox_client()
    if dbx:
        dropbox_paths = [
            "/8825-Jh/8825_core/brain/capability_map.json",
            "/Justin Harmon/Public/8825/8825-Jh/8825_core/brain/capability_map.json",
        ]
        for dbx_path in dropbox_paths:
            content = load_from_dropbox(dbx, dbx_path)
            if content:
                try:
    ...
    # Try local filesystem (when running locally)
    local_paths = [
        BRAIN_DIR.parent.parent.parent / "8825_core/brain/capability_map.json",
        BRAIN_DIR / "capability_map.json",
    ]
...
# For agent registry and specs, use similar approach
def load_agent_registry() -> dict:
    """Load agent registry from Dropbox or local filesystem."""
    dbx = get_dropbox_client()
    if dbx:
        dropbox_paths = [
            "/8825-Jh/8825_core/agents/registry/agent_registry.json",
            "/Justin Harmon/Public/8825/8825-Jh/8825_core/agents/registry/agent_registry.json",
        ]
        for dbx_path in dropbox_paths:
            content = load_from_dropbox(dbx, dbx_path)
            if content:
    ...
    local_paths = [
        BRAIN_DIR.parent.parent.parent / "8825_core/agents/registry/agent_registry.json",
        BRAIN_DIR / "agent_registry.json",
    ]
    for local_path in local_paths:
        if local_path.exists():
            try:
...
def load_agent_spec(agent_name: str) -> dict:
    """Load a specific agent's YAML spec from Dropbox or local filesystem."""
    import yaml
    ...
    if dbx:
        dropbox_paths = [
            f"/8825-Jh/8825_core/agents/specs/{agent_name}.yaml",
            f"/Justin Harmon/Public/8825/8825-Jh/8825_core/agents/specs/{agent_name}.yaml",
        ]
        for dbx_path in dropbox_paths:
            content = load_from_dropbox(dbx, dbx_path)
            if content:
    ...
    # Try local filesystem
    local_paths = [
        BRAIN_DIR.parent.parent.parent / f"8825_core/agents/specs/{agent_name}.yaml",
        BRAIN_DIR / f"specs/{agent_name}.yaml",
    ]
    for local_path in local_paths:
        if local_path.exists():
            try:
...
def query_dli_for_tool(need: str) -> dict:
    ...
    try:
        ...
        intent_to_cap = {
            ...
            'parseable': ['export_docx'],  # Structured output → export
        }
...
def get_context_from_dli(topic: str, focus: str = "global") -> dict:
    ...
    In cloud deployment, returns a message indicating context requires local 8825 connection.
    When running locally, uses Context Builder and DLI.
    """
    import os
    
    # Check if running in cloud (Replit) vs local
    is_cloud = os.environ.get("REPLIT") or os.environ.get("REPL_ID")
    
    if is_cloud:
        # Cloud deployment - context requires local 8825 connection
        return {
            "topic": topic,
            "focus": focus,
            "source": "cloud_stub",
            "context": f"Context for '{topic}' requires connection to local 8825 system. "
                       f"Use jh_brain_query to find the right tool, or ask about capabilities.",
            "note": "Full context retrieval available when connected to local 8825 instance.",
            "available_actions": [
                "jh_brain_query - Find the right tool for a task",
                "jh_brain_guidance - Get philosophy-based guidance",
                "jh_brain_list_capabilities - See all available capabilities"
            ]
        }
    
    # Local deployment - use full Context Builder / DLI
    import subprocess
...
def main():
    ...
    print("✅ Jh Brain MCP initialized", file=sys.stderr)
...
def run_http_server(port: int = 5171):
    ...
    def orchestrate(request: ContextRequest):
        """
        Full orchestration: preflight → decide strategy → execute → return results.
    ...
    print(f"🧠 Jh Brain HTTP server starting on port {port}", file=sys.stderr)
